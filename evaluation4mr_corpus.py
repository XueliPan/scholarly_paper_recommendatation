...
def rename_rank_item(data):
    ...
    ranking_df.to_csv('rank_result_rm/rank_result_mr_CP.csv', index=False)
    return ranking_df
...
def iter_rank_ls(ranking_df, ground_truth):
    ...
    for r in range(50):
        ...
        pd.DataFrame.from_dict(rank_matrix).to_csv('rank_result_rm/rank_matrix_mr_CP.csv', index=False)
    return rank_matrix
...
data = 'rank_result_rm/rank_result_mr_own_corpus.csv'
