...
2. pretrain word2vec model GoogleNews-vectors-negative300.bin.gz and candidate_paper.csv are available via google
drive link, you can download the files and change the path in this script so as to run the script successfully.
3. result saved in rank_result_rm/rank_result_mr_GoogleNews.csv
"""
...
# load pre-train model on my Google News corpus via API
...
new_df.to_csv('rank_result_rm/rank_result_mr_GoogleNews.csv', index=False)
